...
function EditGroupForm({ onClose, name, avatar_url }: Props) {
  const [groupName, setGroupName] = useState(name);
  const [avatarPreview, setAvatarPreview] = useState<string>(
    avatar_url ?? "/assets/group.png",
  );
  const [openAddGroupMember, setOpenAddGroupMember] = useState<boolean>(false);
  ...
  const handleCloseAddGroupMember = () => {
    setOpenAddGroupMember(false);
  };
  ...
  return (
    <>
      {!openAddGroupMember ? (
        ...
        <AddGroupMembersForm
          conversationId={memberList.conversation_id}
          onClose={handleCloseAddGroupMember}
        />
      )}
    </>
  );
