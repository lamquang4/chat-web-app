      ...
      <div className="flex flex-col h-full gap-4">
        <div className="flex flex-col items-center gap-4 w-full">
          ...
          <div className="space-y-[5px] w-full">
            ...
            <Input
              ...
              maxLength={50}
              className="w-full font-medium bg-transparent border-b border-gray-200 focus:border-primary pb-1 transition-colors"
            />
          </div>
        </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-2 overflow-x-auto pb-1">
              {selected.map((f) => (
                <div
                  key={f.user_id}
                  className="flex items-center gap-2 bg-secondary rounded-md p-2 shrink-0"
                >
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="overflow-y-auto flex-1">
            {friends.length === 0 ? (
              <p className="text-center py-2">Không tìm thấy kết quả</p>
            ) : (
              friends.map((friend) => (
                <FriendSelectItem
                  key={friend.user_id}
